...
export function attachTerminalListener(events: IEventBus, renderer: TerminalRenderer, version: string): void {
  ...
  events.on('PASS_COMPLETED', (evt: AgenticEvent) => {
    ...
    if (evt.payload) {
      ...
      const files = evt.payload.files as ReadonlyArray<{ status: string; file: string }> | undefined;
      if (files && files.length > 0) {
        renderer.logChangedFiles(files);
      } else if (evt.pass !== undefined && evt.pass >= PipelinePass.Refactor && evt.pass <= PipelinePass.Security) {
  ...
  events.on('SELF_CORRECTION_ATTEMPTED', (evt: AgenticEvent) => renderer.logCompaction(evt.message));
  ...
  events.on('PIPELINE_PAUSED', (evt: AgenticEvent) => renderer.pausedBanner(evt.message));

  events.on('PIPELINE_RESUMED', (evt: AgenticEvent) => renderer.resumedBanner(evt.message));

  events.on('PIPELINE_COMPLETED', (_: AgenticEvent) => renderer.logPipelineComplete(version));
}
