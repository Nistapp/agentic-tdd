...
| **Digital twin / traceability** | Changed symbols link back to their design via `@see`, plus `@see`/`SEC:` anchors | Pass 7 `see-link` rule (on targeted symbols); `targetSymbols`/`fileChanges` anchors | [2. Prompt Engineering §3.5](../contributor-deep-dive/02-prompt-engineering.md#35-separation-of-concerns-rules-per-pass) |
