...
| P-2 | **Model routing strategy** | Routing is **config-driven at runtime** — committed `config.default.json` (openrouter/deepseek-v4-pro for passes 0–2, openrouter/deepseek-v4-flash for passes 3–7, served via the OpenRouter gateway) merged with a git-ignored `.agentic-tdd/config.json` override, plus `--model`/`--config` CLI flags; agent-file frontmatter is the final fallback ([ADR-0009](../adrs/0009-configurable-per-agent-models.md)). | [5. Agent Prompt System](05-agent-prompt-system.md) |
