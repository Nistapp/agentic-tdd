...
|---|---|---|
| **Coding Harness (agent runner)** | The sub-agent that reads the scoped prompt and performs file edits. Executed per pass via the `IAgentRunner` seam — **Pi** (in-process SDK, default) or opencode CLI (`--backend opencode-cli`) — see [ADR-0010](../adrs/0010-agent-agnostic-sdk-architecture.md) and [`src/infrastructure/agent-runners/`](../../../src/infrastructure/agent-runners/). | Shipped |
