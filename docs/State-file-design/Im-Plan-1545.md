...
This plan details how to implement a robust, state-driven context management system. By tracking progress, commit hashes, and file modifications in a version-controlled state file, we can dynamically build the exact context each agent needs, ignoring the legacy "Static Prefix" constraint to maximize accuracy across different LLMs.
...
### Critique & Synthesis
1. **Logging progress and details for better 'resume' context:** **AGREED.** The ability to resume effectively depends entirely on knowing exactly what happened prior. A state file is a hard dependency for this.
...
Update `src/core/types.ts` to include a rich history tracker.
```typescript
...
- Modify `PipelineOrchestrator.#maybeCommit`:
  ...
  - After the commit succeeds, retrieve the `HEAD` commit hash and update `ctx.history[currentPass].commitHash`, then save the state file again (this won't be committed until the *next* pass, which is fine, or we can amend the commit).
- In `#runPass7` (or pipeline completion), delete the state file and commit the deletion.

### 3. The `ContextBuilder` Abstraction
Extract context generation out of `shared.ts` into a dedicated `ContextBuilder` (or extend `AgentContextPayload` logic):
...
- *Future Tweakability:* Because we have the commit hash, the `ContextBuilder` can be easily modified in the future to inject `git diff` outputs into the prompt instead of (or in addition to) full file contents.

### 4. Wire State Store in CLI for Resume
Update `src/cli/index.ts`:
- Remove the brittle `getLastCompletedPass()` git-log parser.
- `--resume` now simply loads `.opencode/state-<feature-name>.json` to instantly restore `PipelineContext`, including all historical file paths and commit hashes.
...
> 
> **Proposed Solution:** The state file committed during Pass X will contain the history up to Pass X-1, plus the `filesTouched` for Pass X. The `commitHash` for Pass X will be added to the state file *immediately after* the commit, meaning it gets tracked in the Git working directory and will be formally committed in the *next* pass (Pass X+1). Is this acceptable, or should we use a Git `post-commit` / `commit --amend` trick?

## Verification Plan
### Automated Tests
- Add tests in `test/orchestrator.test.ts` to verify `ctx.history` is populated correctly with `filesTouched` before commits.
